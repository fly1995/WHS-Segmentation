...
from tensorflow_train.losses.semantic_segmentation_losses import softmax_cross_entropy_with_logits,generalized_dice_loss,dice_softmax, sigmoid_cross_entropy_with_logits
...
class MainLoop(MainLoopBase):
    def __init__(self, modality, cv):
        ...
        self.image_size = [64, 64, 64]#网络输入大小
        if modality == 'ct':
            self.image_spacing = [3, 3, 3]#空间大小，相当于图像实际大小是192*192*192
        else:
            self.image_spacing = [4, 4, 4]#空间大小，相当于图像实际大小是256*256*256
        self.input_gaussian_sigma = 1.0
        self.label_gaussian_sigma = 1.0
        self.use_landmarks = True
        self.output_folder = './output/Proposed_' + modality + '_' + str(cv) + '/' + self.output_folder_timestamp() + '/'
        ...
        self.loss_function = generalized_dice_loss#softmax_cross_entropy_with_logits#sigmoid_cross_entropy_with_logits#softmax_cross_entropy_with_logits#generalized_dice_loss #,dice_softmax
        self.network = LA_GUT_MHD#LA_GUT #CII_Net  #network_proposed #network_unet, network_scn
    ...
    def test(self):
        ...
        for i in range(num_entries):
            ...
            if self.has_validation_groundtruth:
                ...
                run_tuple = self.sess.run((self.prediction_softmax_val, self.local_prediction_val, self.spatial_prediction_val, self.loss_val) + self.val_loss_aggregator.get_update_ops(),feed_dict=feed_dict)
            else:
                feed_dict = {self.data_val: np.expand_dims(generators['image'], axis=0)}#image是文件名后面几个字符
                # run loss and update loss accumulators
