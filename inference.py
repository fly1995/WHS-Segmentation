...
#训练20个mri或ct后，测试40个mri或ct，测试前将nii文件转为mha文件，用reorient.py
from LA_GUT_MHD import *
class MainLoop(MainLoopBase):
    def __init__(self, modality, image_folder, image_postfix, image_extension, landmarks_file, load_model_filename, output_folder):
        ...
        dataset_parameters = dict(image_size=self.image_size,image_spacing=self.image_spacing,
                                  base_folder='',image_folder=self.image_folder,cv=0, modality=self.modality,input_gaussian_sigma=self.input_gaussian_sigma,label_gaussian_sigma=self.label_gaussian_sigma,use_landmarks=self.use_landmarks,num_labels=self.num_labels,
                                  image_filename_postfix=self.image_postfix,image_filename_extension=self.image_extension,landmarks_file=self.landmarks_file,data_format=self.data_format, save_debug_images=self.save_debug_images)
        self.dataset = Dataset(**dataset_parameters)
        self.dataset_inference = self.dataset.dataset_inference()
        self.network = LA_GUT_MHD#network_scn#network_scn#LA_GUT_MHD # network_unet # network_unet , CII_Net
    ...
    def test(self):
        ...
        segmentation_test = SegmentationTest(labels,channel_axis=channel_axis,interpolator='cubic', largest_connected_component=False, all_labels_are_connected=False)#需要后处理LCC为true
...
if __name__ == '__main__':
    ...
    image_folder = 'mmwhs_dataset/ct_cv_fold1/'  # e.g., 'mmwhs_dataset/mr_mha/'
    image_postfix = '_image'
    image_extension = '.mha'
    landmarks_file = 'mmwhs_dataset/setup/ct_seg_center_rai_w_spacing.csv'  # if None, center images; e.g., 'mmwhs_dataset/setup/mr_seg_center_rai_w_spacing.csv'
    load_model_filename = './output/Proposed_ct_1/2024-11-15_15-08-16/weights/model-50000'
    output_folder = 'E:/D6_MMWHS_2017/Cross_Validation20/Proposed/CT/'
    loop = MainLoop(modality, image_folder, image_postfix, image_extension, landmarks_file, load_model_filename, output_folder)
    loop.run_test()
